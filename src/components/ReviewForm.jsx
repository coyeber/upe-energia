...
const tariffFields=[['consumo_kwh','Consumo (kWh)'],['custo_total','Custo do posto (R$)'],['te_valor','TE (R$)'],['tusd_valor','TUSD (R$)'],['te_tarifa','Tarifa TE (R$/kWh)'],['tusd_tarifa','Tarifa TUSD (R$/kWh)'],['demanda_faturada_kw','Demanda faturada (kW)']]
...
export default function ReviewForm({data,onSave,fileName,provider,model}){
 ...
 return <section className="card review-card animate-enter">
  <div className="review-head"><div><div className="label text-[#123b73]">Conferência obrigatória</div><h2 className="section-title">Revise os dados antes de alimentar o dashboard</h2><p className="section-subtitle">O sistema organiza a fatura automaticamente. Confirme especialmente Ponta, Fora de Ponta, TE, TUSD, consumo faturado e demanda faturada.</p></div><span className="review-badge"><ShieldCheck size={15}/>Análise + validação</span></div>
  ...
  <div className="review-warning"><CheckCircle2 size={17}/><div><strong>Antes de salvar</strong><p>Não misture Ponta e Fora de Ponta. Se a fatura não informar determinado campo, deixe-o vazio. TE e TUSD também devem permanecer separados.</p></div></div>

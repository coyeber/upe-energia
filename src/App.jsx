...
import { Activity, Building2, Cpu, MapPin, ShieldCheck, Sparkles, Wifi, Zap } from 'lucide-react'
...
export default function App(){
 const [page,setPage]=useState('dashboard'),[bills,setBills]=useState(()=>getBills()),[status,setStatus]=useState('checking'),[health,setHealth]=useState(null),[boot,setBoot]=useState(true)
 useEffect(()=>{const min=new Promise(r=>setTimeout(r,2100));const healthReq=checkAnalysisService().then(h=>{setHealth(h);setStatus(h.apiKeyConfigured?'ready':'error')}).catch(()=>setStatus('error'));Promise.allSettled([min,healthReq]).then(()=>setBoot(false))},[])
...
function BootScreen(){
 ...
 useEffect(()=>{const id=setInterval(()=>setProgress(p=>Math.min(96,p+(p<45?7:p<78?4:2))),105);return()=>clearInterval(id)},[])
 ...
 return <div className="boot-screen boot-electric"><div className="boot-grid"/><div className="boot-glow"/><div className="boot-glow boot-glow-blue"/><div className="boot-bolt-trail trail-a"/><div className="boot-bolt-trail trail-b"/><div className="boot-card electric-boot-card"><div className="boot-brand-lockup"><div className="boot-mini-bolt"><Zap size={24} fill="currentColor"/></div><img src="/upe-logo.png" alt="UPE"/><span/><div><strong>POLI Energia</strong><small>Centro de Análise Energética</small></div></div><div className="lightning-reactor"><div className="reactor-orbit orbit-a"/><div className="reactor-orbit orbit-b"/><div className="reactor-core"><Zap size={56} strokeWidth={1.8} fill="currentColor"/></div><span className="reactor-spark s1"/><span className="reactor-spark s2"/><span className="reactor-spark s3"/><span className="reactor-spark s4"/></div><div className="boot-eyebrow"><Sparkles size={14}/> SISTEMA ENERGIZANDO</div><h1>Análise que transforma energia em decisão.</h1><p>Preparando indicadores de consumo, demanda, Ponta, Fora de Ponta, TE, TUSD e custos da POLI.</p><div className="boot-stage-row"><span>{stage}</span><strong>{Math.round(progress)}%</strong></div><div className="boot-progress electric-progress"><span style={{width:`${progress}%`}}/></div><div className="boot-tech-row"><span><ShieldCheck size={14}/> Ambiente seguro</span><span><Cpu size={14}/> Serviço de análise ativo</span><span><Activity size={14}/> Indicadores sincronizados</span></div></div></div>
}

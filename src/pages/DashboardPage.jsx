...
function Hero({onAdd,bills}){return <section className="dashboard-hero animate-enter"><div className="hero-grid"/><div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-5"><div><div className="hero-kicker"><Sparkles size={14}/>Centro de análise energética</div><h1>Dashboard de Energia UPE</h1><p>Consumo, gastos, dias de faturamento, demanda, tendências e recomendações em uma visão executiva.</p><div className="flex flex-wrap gap-2 mt-4"><span className="hero-chip"><ShieldCheck size={14}/>Análise automatizada + validação técnica</span><span className="hero-chip"><Database size={14}/>{bills.length} conta{bills.length===1?'':'s'} confirmada{bills.length===1?'':'s'}</span></div></div><button className="hero-button" onClick={onAdd}><Plus size={19}/>Adicionar conta</button></div></section>}
...
function EmptyState({onAdd}){return <section className="card empty-premium animate-enter delay-2"><div className="empty-logo"><img src="/upe-logo.png"/></div><div className="label text-[#123b73]">Base vazia</div><h2>Comece pela primeira conta de energia</h2><p>Ao enviar um PDF, o sistema identifica os dados, você confere os campos e o dashboard monta automaticamente os períodos Mês, Ano e Todos os anos.</p><button className="btn-red mt-5" onClick={onAdd}><Plus size={18}/>Analisar primeira conta</button></section>}

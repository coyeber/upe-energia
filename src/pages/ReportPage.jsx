...
export default function ReportPage({bills}){
 ...
 return <div className="space-y-5 page-enter report-page">
  ...
  <section className="grid xl:grid-cols-[1.35fr_.65fr] gap-5">
   ...
   <div className="card p-5 md:p-6"><div className="label text-[#ed1c2e]">Leitura executiva</div><h2 className="section-title">Situação mais recente</h2><div className="report-latest"><span>{monthLong(latest.mes_referencia)}</span><strong>{num(latest.consumo_kwh)} kWh</strong><em>{brl(latest.valor_total)}</em></div><p className="text-sm text-slate-600 leading-relaxed mt-4">{latest.diagnostico?.leitura_executiva||latest.diagnostico?.resumo||'Sem síntese executiva retornada pelo Gemini.'}</p><div className="report-quality mt-5"><div><span>Consumo</span><strong>{coverage.consumptionMonths} meses</strong></div><div><span>Financeiro</span><strong>{coverage.valueMonths} meses</strong></div><div><span>Demanda</span><strong>{coverage.demandMonths} meses</strong></div></div></div>
  </section>
  ...
  <section className="report-disclaimer"><BrainCircuit size={18}/><div><strong>Como interpretar este relatório</strong><p>Os números do painel vêm das contas confirmadas e do histórico extraído dos PDFs. Recomendações da IA são apoio à decisão e devem ser validadas com medições, contratos, tarifas e inspeções em campo antes de qualquer intervenção.</p></div></section>
 </div>
}

...
export default function AnalysisPage({bills}){
 ...
 const unitCost=current.valor_total&&baseConsumption?current.valor_total/baseConsumption:null
 ...
 const facts=[
  {icon:BadgeDollarSign,title:'Custo efetivo total',value:unitCost?`${brl(unitCost)}/kWh`:'Não calculável',text:'Indicador global da fatura. Para gestão tarifária, use os custos separados de Ponta e Fora de Ponta.'},
  ...
  {icon:Landmark,title:'Multas + impostos',value:(fin.multasTotal!=null||fin.impostosTotal!=null)?brl((fin.multasTotal||0)+(fin.impostosTotal||0)):'Não informado',text:`Multas: ${brl(fin.multasTotal)} • Impostos: ${brl(fin.impostosTotal)}.`},
...
function TariffAnalysis({title,subtitle,data={}}){const effective=data.consumoKwh>0&&data.custoTotal!=null?data.custoTotal/data.consumoKwh:null;return <div className="tariff-card blue"><div className="tariff-card-head"><div><span>{title}</span><small>{subtitle}</small></div><Clock3 size={20}/></div><div className="tariff-card-grid"><Row label="Consumo" value={data.consumoKwh==null?'—':`${num(data.consumoKwh)} kWh`}/><Row label="Custo" value={brl(data.custoTotal)}/><Row label="TE" value={brl(data.teValor)}/><Row label="TUSD" value={brl(data.tusdValor)}/><Row label="Custo efetivo do posto" value={effective==null?'—':`${brl(effective)}/kWh`}/></div></div>}
